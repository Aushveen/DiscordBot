const {Client, Intents, Emoji, MessageActionRow, MessageButton, SystemChannelFlags} = require('discord.js');
...
function makeGrid() {
    ...
    for(let row = 0; row < 3; row++){
        ...
        for(let col = 0; col < 3; col++){
            messageButton = new MessageButton().setCustomId('tictactoe_' + row + '_' + col)
...
client.on('interactionCreate', async interaction => {
    ...
    if(tictactoe_state[row][col] != EMPTY){
        interaction.update({
            content: "You can't select that spot!",
            components: makeGrid()
        })
        return;
    }
